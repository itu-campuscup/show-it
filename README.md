...
Show IT consumes `index.json` plus `rankings/{year}/{beer|spin|sail}.json` for the three ranking pages. `/teams/` consumes `teams/index.json`, whose radar values are generated by the same team-profile calculation as Judge IT.
